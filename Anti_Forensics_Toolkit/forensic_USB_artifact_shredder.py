...
def overwrite_usb_registry_key_identification():
    ...
    try:
        ...
        for i in range(0, winreg.QueryInfoKey(reg_key)[0]):
            ...
            list_of_devices.append(r"SYSTEM\CurrentControlSet\Enum\USBSTOR" + "\\" + device)
            # list all subkeys
        for device in list_of_devices:
            open_subkey = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, device, 0, winreg.KEY_ALL_ACCESS)
            for j in range(0, winreg.QueryInfoKey(open_subkey)[0]):
                serial_number = winreg.EnumKey(open_subkey, j)
                list_of_serial_numbers.append(device + "\\" + serial_number)
                # query values
        for serial_number in list_of_serial_numbers:
            open_subkey = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, serial_number, 0, winreg.KEY_ALL_ACCESS)
